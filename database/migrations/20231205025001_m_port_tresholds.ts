...
export async function up(knex: Knex): Promise<void> {
    return knex.schema.createTable('m_port_tresholds', function (table) {
        table.increments('id').primary().notNullable();
        table.uuid('uuid').unique().notNullable();
        table.integer('id_m_severity').unsigned().references('m_severity.id').notNullable();
        table.integer('id_m_port').unsigned().references('m_ports.id').notNullable();
...
export async function down(knex: Knex): Promise<void> {
    return knex.schema.dropTable('m_port_tresholds');
}